...
@Component({
  selector: 'app-dash-all-year',
  templateUrl: './dash-all-year.component.html',
  styleUrls: ['./dash-all-year.component.scss']
})
export class DashAllYearComponent implements OnInit{
  ...
  getPie() {
   ...
   getNetAPayer() {
    ...
    this.dashAllService.masseSalarialYearPrecedement(this.currentUser.code_entreprise).subscribe(
        res =>  {
        ...
        this.dashAllService.allocationYear(this.currentUser.code_entreprise).subscribe(
            res => {
                ...
                this.chartOptionAllocations = {
                    ...
                    xaxis: { 
                        categories: this.allocationList.map((item: any) => item.year),  
                        labels: {
                            style: {
                                colors: "#a9a9c8",
                                fontSize: "14px"
    ...
    getPiePresence() {
        this.dashAllService.presencePieYEAR(this.currentUser.code_entreprise).subscribe(
            res => {
    ...
    getRecrutement() {
        this.dashAllService.recrutementsTotalYear(this.currentUser.code_entreprise).subscribe(
            res =>  {
                this.recrutementsList = res;
                this.recrutementsList.map((item: any) => this.recrutements = parseFloat(item.count));
            }
        );
        this.dashAllService.postulantsTotalYear(this.currentUser.code_entreprise).subscribe(
            res =>  {
                this.postulantsList = res;
                this.postulantsList.map((item: any) => this.postulants = parseFloat(item.count));
            }
        );
        this.dashAllService.postulantsRetenuTotalYear(this.currentUser.code_entreprise).subscribe(
            res =>  {
                this.recrusList = res;
